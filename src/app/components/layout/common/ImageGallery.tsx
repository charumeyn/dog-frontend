...
const ImageGallery: React.FunctionComponent<ImageGalleryProps> = ({ images, mainImage, isSquare, isHorizontal }) => {
  ...
  return (
    <div className={`${isHorizontal ? "flex flex-col-reverse" : "grid grid-cols-8 gap-4"}`}>
      <div className={`${isHorizontal ? "grid grid-cols-8 gap-4 mt-4" : "col-span-1"}`}>
        {images?.map((image, i) => (
          <img key={i} className={`${isHorizontal ? "" : "mb-4"} aspect-[5/5] object-cover rounded-xl hover:cursor-pointer`}
