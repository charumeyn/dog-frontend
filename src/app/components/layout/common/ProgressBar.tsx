    ...
    fundraiser ?
      <div className={`${classNames ? classNames : ""}`}>
        <div className="flex w-full font-medium items-center">
          ...
          <div className="w-14 text-right text-sm font-medium text-orange-600">
            {percent}
          </div>
        </div>
        <p className="text-sm text-zinc-500 mt-1"><span className="font-medium">${total}</span> raised of ${fundraiser?.goalAmount} </p>
      </div> : null
  )
}
